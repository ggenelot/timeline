  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col gap-2 p-2">
        <Group title="Engagé" count={engaged.length} emptyLabel="Aucun matériel engagé">
          {engaged.map((m) => (
            <MaterielChip key={m.container_type_id} name={m.name} code={m.code} color={m.category?.color ?? null} />
          ))}
        </Group>
        <Group title="Disponible" count={available.length} emptyLabel="Aucun contenant disponible">
          {available.map((c) => (
            <MaterielChip key={c.id} name={c.name} code={c.code} color={c.category?.color ?? null} />
          ))}
        </Group>
        <Group title="Indisponible" count={unavailable.length} emptyLabel="Aucun contenant indisponible">
          {unavailable.map((c) => (
            <MaterielChip
              key={c.id}
              name={c.name}
