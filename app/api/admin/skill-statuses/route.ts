...
export async function POST(req: NextRequest) {
  ...
  const body = (await req.json()) as { label?: string; color?: string; mark?: string; is_validating?: boolean };
  ...
  const { data: existing } = await client!.from('skill_statuses').select('key').like('key', `${key}%`);
  ...
  const { data, error: dbError } = await client!
    ...
    .insert({
      ...
      mark: body.mark?.trim() || '✓',
      is_validating: body.is_validating ?? false,
