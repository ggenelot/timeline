...
export async function PATCH(request: NextRequest, { params }: { params: { type: string } }) {
  ...
  if (typeof payload.enabled !== 'boolean') {
    return NextResponse.json({ error: 'Le champ enabled (booléen) est requis.' }, { status: 400 });
  }
  ...
  const { error } = await serviceClient
    .from('slack_message_templates')
    .update({ enabled: payload.enabled, updated_by: auth.user.id, updated_at: new Date().toISOString() })
    .eq('type', type);
  ...
  return NextResponse.json({ success: true, enabled: payload.enabled });
}
